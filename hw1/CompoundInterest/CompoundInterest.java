...
public class CompoundInterest {
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        return CompoundInterest.futureValue(presentValue, rate, targetYear) * Math.pow((100-inflationRate)/100, numYears(targetYear));
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        double total = 0;
        for (int i = CompoundInterest.numYears(targetYear); i >=0 ; i = i - 1) {
            total += perYear * Math.pow((100 + rate)/100, i);
        }
        return total;
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        return CompoundInterest.totalSavings(perYear, targetYear, rate) * Math.pow((100 - inflationRate)/100, numYears(targetYear));
    }
